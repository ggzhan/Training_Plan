...
function toggleEditMode(button) {
    ...
    if (!isEditing) {
        ...
        // Initialize previous values for all selects (players and unpaired)
        const allSelects = exerciseCard.querySelectorAll('.player-select, .unpaired-player-select');
        allSelects.forEach(s => {
            if (!s.dataset.previousValue) {
                s.dataset.previousValue = s.value;
            }
        });

        // Attach listener once
        if (!exerciseCard.dataset.listenersAttached) {
            ...
            exerciseCard.addEventListener('change', (e) => {
                ...
                if (e.target.classList.contains('player-select')) {
                    ...
                    if (oldPlayer && oldPlayer !== newPlayer) {
                        ...
                        otherSelects.forEach(sel => {
                            if (sel.value === newPlayer) {
                                sel.value = oldPlayer;
                                sel.dataset.previousValue = oldPlayer;
                            }
                        });
                    }
                ...
                // Unpaired selects
                if (e.target.classList.contains('unpaired-player-select')) {
                    const newPlayer = e.target.value;
                    const oldPlayer = e.target.dataset.previousValue;
                    if (oldPlayer && oldPlayer !== newPlayer) {
                        const otherSelects = Array.from(exerciseCard.querySelectorAll('.unpaired-player-select')).filter(s => s !== e.target);
                        otherSelects.forEach(sel => {
                            if (sel.value === newPlayer) {
                                sel.value = oldPlayer;
                                sel.dataset.previousValue = oldPlayer;
